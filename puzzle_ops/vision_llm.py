...
class OpenAIVisionLLMClient:
    ...
    def analyze(self, images: list[dict[str, object]], country: str, category: str, local_summary: LocalVisualSummary) -> VisionLLMResult:
        ...
        return VisionLLMResult(
            ...
            culture_elements=tuple(str(item) for item in data.get("culture_elements", []) if item),
            style=str(data.get("style", "")),
            risk_tags=tuple(str(item) for item in data.get("risk_tags", []) if item),
            prompt_keywords=tuple(str(item) for item in data.get("prompt_keywords", []) if item),
